...
def parse_options():
    ...
    # Soppa: module support
    parser.add_option(
        make_option('-m', '--modules',
            default=[],
            dest='m',
...
Remember that -f can be used to specify fabfile path, and use -h for help.""")
        ...
        actions = (options.list_commands, options.shortlist, options.display,
            arguments, remainder_arguments, default, options.m)
        if not any(actions):
            parser.print_help()
            sys.exit(1)
        ...
        if not (arguments or remainder_arguments or default or options.m):
            parser.print_help()
        ...
        # Soppa
        # NOTE: order is lost, if mixing @tasks and -m; should not mix, or mix with care.
        def soppa_cli_runner(cls, method):
            instance = cls()
            getattr(instance, method)()
        if options.m:
            from soppa.contrib import NeedMixin
            actions = options.m.split(',')
            for action in actions:
                # foo.bar, foo.bar:arg, foo.bar:key=value
                module, method = action.split('.')
                cls = NeedMixin().load(module)
                execute(
                    soppa_cli_runner,
                    hosts=options.hosts,
                    roles=options.roles,
